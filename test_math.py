...
from gas_gs import compute_predicted_projections
...
def test_multiple_gaussians_and_rays(num_gaussians, num_rays):
    ...
    # Parámetros aleatorios (requieren gradiente para optimización)
    means = torch.rand(num_gaussians, 2, requires_grad=True)
    # Truco para hacer matriz def. positiva simétrica
    ...
    # Forward pass
    proj_pred = compute_predicted_projections(means, inv_sigmas, p_rays, u_rays)
    ...
    print(f"Mean gradient shape: {means.grad.shape}")
    print(f"Covariance inverse gradient shape: {inv_sigmas.grad.shape}")
...
def test_integral_value():
    ...
    proj_pred = compute_predicted_projections(mean, inv_sigma, p_ray, u_ray)
